  ...
  <div>
    ...
    <ul class="rank-list">
       <li v-for="(item,ind) in rank" :key="item.phone">
           <img src="@/assets/image/list-rank1.png" v-if="ind == 0">
           <img src="@/assets/image/list-rank2.png" v-else-if="ind == 1">
    ...
    methods:{
      ...
      first(){
        if(this.page.pageNo == 1){
          layx.msg('已经在第一页',{dialogIcon:'warn',position:'ct'})
        }else{
          this.initPage(productType,1,9)
        }
      },
      last(){
        if(this.page.pageNo == this.page.totalPage){
          layx.msg('已经是最后一页',{dialogIcon:'warn',position:'ct'})
        }else{
      ...
      pre(){
        if(this.page.pageNo == 1){
          layx.msg('已经在第一页，无法向前跳转',{dialogIcon:'warn',position:'ct'})
        }else{
      ...
      next(){
        if(this.page.pageNo == this.page.totalPage){
          layx.msg('已经是最后一页，无法向后跳转',{dialogIcon:'warn',position:'ct'})
        }else{
          this.initPage(productType,this.page.pageNo+1,9)
        }
      },
